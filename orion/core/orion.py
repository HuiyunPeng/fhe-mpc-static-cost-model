...
from .primitive_tracer import PrimitiveTracer
...
class Scheme:
    ...
    def init_scheme(self, config: Union[str, Dict[str, Any]]):
        ...
        self.trace_logger = PrimitiveTracer(
            enabled=self.params.get_trace_enabled(),
            output_path=self.params.get_trace_path(),
        )
    ...
    def save_trace(self, path: str = None):
        """
        Persist the active primitive trace (if enabled) to disk.
        Returns the resolved path or None when tracing is disabled.
        """
        if self.trace_logger:
            return self.trace_logger.save(path)
        return None
