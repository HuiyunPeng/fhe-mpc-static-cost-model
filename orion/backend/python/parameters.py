...
class OrionParameters:
    ...
    def __str__(self) -> str:
        ...
        if self.trace_primitives:
            output.append("  Primitive tracing: enabled")
            if self.trace_path:
                output.append(f"  Trace path: {self.trace_path}")
...
class NewParameters:
    ...
    def get_trace_enabled(self) -> bool:
        return self.orion_params.trace_primitives

    def get_trace_path(self):
        path = (
            self.orion_params.trace_path
            if self.orion_params.trace_path
            else "orion_primitive_trace.json"
        )
        return os.path.abspath(os.path.join(os.getcwd(), path))
