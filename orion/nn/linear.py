...
class LinearTransform(Module):
    ...
    def evaluate_transforms(self, x):
        trace_params = {
            "level": self.level,
            "depth": self.depth,
            "input_shape": tuple(getattr(self, "input_shape", []) or []),
            "output_shape": tuple(getattr(self, "output_shape", []) or []),
            "fhe_input_shape": tuple(getattr(self, "fhe_input_shape", []) or []),
            "fhe_output_shape": tuple(getattr(self, "fhe_output_shape", []) or []),
            "bsgs_ratio": self.bsgs_ratio,
        }
        ...
        with self._trace_context(params=trace_params):
